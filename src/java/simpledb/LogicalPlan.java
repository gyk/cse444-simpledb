...
//    private Query owner;
     ...
     * Catalog#getDatabaseFile(int)}). Aliases are added as base tables are added via {@link #addScan}.
     *
     ...
     *
     * @param field         The name of the over which the filter applies; this can be a fully qualified field
     *                      (tablename.field or alias.field), or can be a unique field name without a tablename
     *                      qualifier.  If it is an ambiguous name, it will throw a ParsingException
     * @param p             The predicate for the filter
     * @param constantValue the constant to compare the predicate against; if field is an integer field, this should be
     *                      a String representing an integer
     * @throws ParsingException if field is not in one of the tables added via {@link #addScan} or if field is ambiguous
     *                          (e.g., two tables contain a field named field.)
     */
     ...
     *
     * @param joinField1 The name of the first join field; this can be a fully qualified name (e.g., tableName.field or
     *                   alias.field) or may be an unqualified unique field name.  If the name is ambiguous or unknown,
     *                   a ParsingException will be thrown.
     * @param joinField2 The name of the second join field
     * @param pred       The join predicate
     * @throws ParsingException if either of the fields is ambiguous, or is not in one of the tables added via {@link
     *                          #addScan}
     */
     ...
     *                   alias.field) or may be an unqualified unique field name.  If the name is ambiguous or unknown,
     *                   a ParsingException will be thrown.
     * @param joinField2 the subquery to join with -- the join field of the subquery is the first field in the result
     *                   set of the query
     * @param pred       The join predicate.
     * @throws ParsingException if either of the fields is ambiguous, or is not in one of the tables added via {@link
     *                          #addScan}
     ...
     *
     * @return A fully qualified name of the form tableAlias.name.  If the name parameter is already qualified with a
     * table name, simply returns name.
     * @throws ParsingException if the field cannot be found in any of the tables, or if the field is ambiguous (appears
     *                          in multiple tables)
     */
...
//                int id = 
     ...
     * @param baseTableStats a HashMap providing a {@link TableStats} object for each table used in the LogicalPlan.
     *                       This should have one entry for each table referenced by the plan, not one entry for each
     *                       table alias (so a table t aliases as t1 and t2 would have just one entry with key 't' in
     *                       this HashMap).
     * @param explain        flag indicating whether output visualizing the physical query plan should be given.
